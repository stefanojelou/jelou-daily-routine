...
### 2026-08-12 — 🆕 NEW — Credit-toppers and subscribers are ~94% DISJOINT populations, not two stages of one funnel

Answering the backlog question directly on the clean co-existing window (Jun 22-Aug 12, prod, unique, 45d conversion): of 35 credit-toppers only 2 (6%) ever fire subscription_changed, and of 25 subscribers only 2 (8%) ever fire credit_topup. So ~94% of each monetization group never touches the other -- credit top-ups and subscriptions are two largely SEPARATE motions (different buyers), not sequential stages where topping up leads to subscribing or vice-versa. This refines the prior lifetime-confounded estimate (3% topup->sub, 14% sub->topup, both distorted by subscription_changed only starting ~Jun 24): on the fair co-existing window the two directions are near-symmetric and tiny (6% / 8%). Time twist (directional, N=2 each): the rare topup->sub crossovers took ~11.9 days; the rare sub->topup crossovers happened same-session (~28 min).

- **Metric:** credit-topper -> subscription_changed 45d conversion (coexist window); sub->topup is 8% = **6**
- **Theme/angle:** Monetization & pay-drivers — Are credit-toppers and subscribers a funnel or disjoint populations -- co-existing-window bidirectional overlap
- **Segment:** overall  ·  **Sources:** mixpanel
- **Confidence:** medium
- **Caveat:** Mixpanel funnels, prod (environment==production), countType=unique, 45-day conversion window, dateRange Jun 22-Aug 12 2026 (co-existing window). credit_topup->subscription_changed: 35->2 (6%); subscription_changed->credit_topup: 25->2 (8%). Ordered funnels catch only A-before-B, so total any-order overlap is at most ~2-4 users out of a combined ~56-58 distinct payers -- either way the populations are ~90%+ disjoint. Small absolute N (35 / 25) so the exact 6%/8% are directional, but the qualitative DISJOINT conclusion is robust (crossover is single-digit users in both directions). avg crossover times (topup->sub ~1,028,676s ~=11.9d; sub->topup ~1,701s ~=28min) are N=2 each -- anecdotal. credit_topup is pre-settlement intent (credits only); subscription_changed is the conversion event (~newborn since Jun 24); reconciled revenue/MRR is Stripe-only (deferred). distinct_id, not company_id-deduped. Refines the lifetime 3%/14% figure by re-testing on the confound-free co-existing window as the backlog asked.

---

### 2026-08-12 — 🆕 NEW — Credit-toppers retain ~4x better than subscribers on near-term agent activity -- and the two are ~94% disjoint populations, not a funnel

On the clean co-existing window (Jun 22-Aug 12, prod), cohorting on each pay event and measuring return via agent_message_sent, credit-toppers (N=35) out-retain subscribers (N=25) at every early week: same-week 60% vs 26%, wk1 24% vs 6%, wk2 23% vs 6%, wk3 14% vs 8%, wk4 15% vs 9%. So on the only monetization-retention proxy Mixpanel can measure, the credit-topup path marks materially stickier near-term product usage than the subscription path. BIG caveat: agent_message_sent is a poor retention yardstick for SUBSCRIBERS specifically -- only ~1 in 4 subscribers even sends an agent message the same week they subscribe, consistent with subscribers being a different (likely sales-sold/managed) motion whose value shows up as continued MRR (Stripe, deferred), not agent chat -- so this proxy almost certainly UNDER-measures subscriber retention. Honest read: measured by return-to-agent-usage toppers look far stickier, but true subscription retention needs Stripe. This finally attempts the Jul-14-deferred angle now that subscription_changed has ~7wk of history -- unblocked on TIME but still fragile on N.

- **Metric:** subscriber wk1 agent-activity retention, coexist window (vs credit-topper 24) = **6**
- **Theme/angle:** Monetization & pay-drivers — Credit purchase (credit_topup) vs conversion (subscription_changed): which path retains better -- co-existing-window retention curves (agent-activity proxy) + subscriber-proxy-invalidity caveat
- **Segment:** overall (topup vs subscription path)  ·  **Sources:** mixpanel
- **Confidence:** low-medium
- **Caveat:** Mixpanel weekly retention CURVES, prod (environment==production), cohort on the pay event, return event = agent_message_sent, dateRange Jun 22-Aug 12 2026 (the co-existing window; subscription_changed launched ~Jun 24). Populations are TINY and deduplicated: credit_topup 35 unique users, subscription_changed 25 -- ratio ~1.4:1 (was 1.7:1 / 24-vs-14 on the Jul-14 shorter window; populations grew, ratio ~stable). $average weekly rates -- credit_topup: wk0 60%, wk1 24%, wk2 23%, wk3 14%, wk4 15%, wk5 2%; subscription_changed: wk0 26%, wk1 6%, wk2 6%, wk3 8%, wk4 9%, wk5 15% (wk5 tick is re-entry noise at N<25). wk0 (60 vs 26) is the most robust cell (full cohort, least window-truncation); wk1/wk2 amplify the gap but rest on ~1-2 returners out of ~20 mature subscriber births so are directional. KEY VALIDITY CAVEAT: agent_message_sent under-measures subscribers -- only 26% send an agent message even in their subscribe-week, so many subscribers are plausibly managed/enterprise accounts whose retention is MRR-continuation (Stripe, deferred), not agent chat; the topper>subscriber gap is partly a measurement artifact of the proxy, not proof toppers are better customers. credit_topup fires pre-checkout = intent-to-pay, credits only, NOT reconciled revenue. Unit is distinct_id, NOT company_id-deduped. Updates the Jul-14 finding that declared this comparison unanswerable (subscription_changed then <3wk old); now attemptable on time but N still too small to bank the exact rates -- revisit when subscriber N grows or Stripe is wired. Marks not drives.
